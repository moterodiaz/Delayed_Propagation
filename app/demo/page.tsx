...
export default function DemoPage() {
  ...
  const affected = state?.events[0]?.affectedFlightIds.length ?? 0;
  const total = state ? "$" + Math.round(state.events[0].cost.totalUsd).toLocaleString("en-US") : "—";

  return (
    <div style={{ width: "100vw", height: "100vh", background: "#040c1e", display: "grid", gridTemplateRows: "54px 1fr 40px", overflow: "hidden", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif", color: "#c8e0f8" }}>
      {/* header */}
      ...
      <footer style={{ display: "flex", alignItems: "center", padding: "0 18px", gap: 28, background: "rgba(1,4,12,0.98)", borderTop: "1px solid rgba(0,160,255,0.09)" }}>
        <Stat label="FLIGHTS AFFECTED" val={String(affected)} c="#ff6b35" />
